...
const CONFIG = {
    ...
    // Working embed sources only - tested and verified
    EMBED_SOURCES: [
        {
            name: 'VidSrc Pro',
...
// For TV: open detail with episodes
// ============================================
...
async function buildHomeContent() {
    ...
    // Load rows in parallel batches for speed
    var batch1 = rows.slice(0, 5);
    var batch2 = rows.slice(5, 10);
    var batch3 = rows.slice(10);

    await loadRowBatch(batch1, 0, container);
    loadRowBatch(batch2, 5, container);
    loadRowBatch(batch3, 10, container);
}
...
function makeContentRow(title, items, index) {
    ...
    var isTop = title.includes('Top Rated');
...
function goToCategory(t) {
    if (t.includes('Anime')) navigateTo('anime');
    else if (t.includes('Series')||t.includes('Shows')||t.includes('Air')||t.includes('Drama')) navigateTo('series');
    else navigateTo('movies');
}
...
function setupScrollEvents() {
    window.addEventListener('scroll', function() {
        ...
        if (p !== 'movies' && p !== 'series' && p !== 'anime') return;
