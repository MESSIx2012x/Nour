...
// ============================================
// DETAIL MODAL
// autoPlay = true means user clicked "Play" on a TV show
// ============================================
...
function renderFullModal(data, type, autoPlay) {
    ...
    // If autoPlay, scroll to episodes
    if (autoPlay && type === 'tv') {
        setTimeout(function() {
...
// ============================================
// EPISODES - WITH EPISODE NUMBERS + WATCHED
// ============================================
...
async function loadPageData(type) {
    ...
    try {
        var data;
        if (type==='movies') data = await discoverMovies({ sort_by:getVal('movieSortFilter')||'popularity.desc', with_genres:getVal('movieGenreFilter')||undefined, page:ps.page });
        else if (type==='series') data = await discoverTV({ sort_by:getVal('seriesSortFilter')||'popularity.desc', with_genres:getVal('seriesGenreFilter')||undefined, page:ps.page });
        else if (type==='anime') data = await getAnime({ sort_by:getVal('animeSortFilter')||'popularity.desc', page:ps.page }, getVal('animeTypeFilter')||'tv');
...
function renderGrid(type) {
    var grid = document.getElementById(type==='movies'?'moviesGrid':type==='series'?'seriesGrid':'animeGrid');
    if (!grid) return;
    var ps = state.pages[type];
    var mt = type==='movies'?'movie':type==='anime'?(getVal('animeTypeFilter')==='movie'?'movie':'tv'):'tv';
    grid.innerHTML = ps.items.map(function(item) { item.media_type=item.media_type||mt; return makeCardHTML(item); }).join('');
...
function applyFilters(type) { state.pages[type].page=1; state.pages[type].items=[]; loadPageData(type); }

function resetFilters(type) {
    if (type==='movies') { document.getElementById('movieGenreFilter').value=''; document.getElementById('movieSortFilter').value='popularity.desc'; }
    else if (type==='series') { document.getElementById('seriesGenreFilter').value=''; document.getElementById('seriesSortFilter').value='popularity.desc'; }
    else { document.getElementById('animeTypeFilter').value='tv'; document.getElementById('animeSortFilter').value='popularity.desc'; }
    applyFilters(type);
}
